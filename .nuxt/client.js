        ...
        if (NUXT.serverRendered) {
          ...
          if (NUXT.components) {
            Component.options.components = Object.assign(
              _Component.options.components,
              NUXT.components[index]
            );
          }
...
async function render(to, from, next) {
  ...
  try {
    ...
    _lastPaths = Components.map((Component, i) =>
      compile(to.matched[i].path)(to.params)
    );
...
function fixPrepatch(to, ___) {
  ...
  Vue.nextTick(() => {
    ...
    // Set layout
    let layout = this.$options._nuxt.err
      ? NuxtError.layout
...
async function mountApp(__app) {
  ...
  if (Components.length) {
    ...
    _lastComponentsFiles = Components.map(
      Component => Component.options.__file
    );
  }
